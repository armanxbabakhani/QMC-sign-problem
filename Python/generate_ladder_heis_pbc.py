...
def generate_heisenberg_hamiltonian_file(N):
    ...
    filename="./Inputs/Triangular_Ladder_Heisenberg/Triangular_Heis_n="+N+"_pbc.txt"
    ...
    for i in range(3, int(N)+1):
        for axis in ["X", "Y", "Z"]:
            ...
            if i == int(N):
                interactions.append(f"1.0 {1} {axis} {i} {axis}")
                interactions.append(f"1.0 {1} {axis} {i-1} {axis}")
                interactions.append(f"1.0 {2} {axis} {i} {axis}")
